...
class DecodeDetections_V1(DecodeDetections):
    '''
    A Keras layer to decode the raw SSD prediction output. 
    It is inheritated from DecodeDetections Layer and adding one more options as keep anchorbox.

    Input shape:
        3D tensor of shape `(batch_size, n_boxes, n_classes + 12)`.

    Output shape:
        3D tensor of shape `(batch_size, top_k, 6+4)`. [add 4 columns for 4 anchor indices]
    '''
    ...
    def build(self, input_shape):
        super(DecodeDetections_V1, self).build(input_shape)
    ...
    def call(self, y_pred, mask=None):
        ...
        Returns:
            ...
            if add_anchors is True:
            `[class_id, confidence, anchor boxes , xmin, ymin, xmax, ymax]`.
            if add_anchors is False:
        ...
        if self.add_anchores:
            # Removing variances and add the converted box coordinates to form the decoded predictions tensor.
            if normalized_coords:
                ancbx = y_pred[:,:,-8] * self.tf_img_width
                ancbw = y_pred[:,:,-6] * self.tf_img_width
                ancby = y_pred[:,:,-7] * self.tf_img_height 
                ancbh = y_pred[:,:,-5] * self.tf_img_height 
                anchor_boxes = tf.concat(values=[tf.expand_dims(ancbx,axis=-1),
                                                 tf.expand_dims(ancby,axis=-1),
                                                 tf.expand_dims(ancbw,axis=-1),
                                                 tf.expand_dims(ancbh,axis=-1)],axis=-1)
                # print("#############################################################")
                # print("check output type")
                # print(type(ancbh))
                # print("check output shape")
                # print(ancbh.shape)
                # print(anchor_boxes.shape)
                # anchor_boxes.set_shape([y_pred.shape[0],y_pred.shape[1],4])
                # print(anchor_boxes.shape)
                # print("#############################################################")
            else:
                anchor_boxes = y_pred[...,-8:-4]
            # # convert tensor to numpy array
            # anchor_boxes_np = tf.Session().run(anchor_boxes)
            # if self.coords == 'centroids':
            #     anchor_boxes_np = convert_coordinates(anchor_boxes_np, start_index=0, conversion='centroids2corners', border_pixels='half')
