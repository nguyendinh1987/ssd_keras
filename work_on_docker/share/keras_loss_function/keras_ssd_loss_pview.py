...
class SSDLoss_Pview:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        ##################################################
        # Because we add bbx score so we cannot use log_loss
        # We change it to smooth_L1_loss for more accurate
        ###################################################
        classification_loss = tf.to_float(self.log_loss(y_true[:,:,:-12], y_pred[:,:,:-12])) # Output shape: (batch_size, n_boxes)
        # classification_loss = tf.to_float(self.smooth_L1_loss(y_true[:,:,:-12], y_pred[:,:,:-12])) # Output shape: (batch_size, n_boxes)
        ...
        n_positive = tf.reduce_sum(positives) # Dinh: This one also need to replace because we replace label by object score.
        # Dinh: it should be : n_positive = tf.count_nonzero(positives,dtype=tf.int32)
