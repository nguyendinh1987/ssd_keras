...
def match_bipartite_greedy_pview(weight_matrix_1, weight_matrix_2, pos_threshold=0.2):
    '''
...
def clip_boxes(boxes1,boxes2):
    '''
    This function is to clip boxes1 so that they are inside boxes2
    boxes have format [xmin,ymin,xmax,ymax]
    '''
    boxes1[:,0] = np.max(np.concatenate([np.expand_dims(boxes1[:,0],axis=-1),
                                         np.expand_dims(boxes2[:,0],axis=-1)],axis=-1),axis=-1)
    boxes1[:,1] = np.max(np.concatenate([np.expand_dims(boxes1[:,1],axis=-1),
                                         np.expand_dims(boxes2[:,1],axis=-1)],axis=-1),axis=-1)
    boxes1[:,2] = np.min(np.concatenate([np.expand_dims(boxes1[:,2],axis=-1),
                                         np.expand_dims(boxes2[:,2],axis=-1)],axis=-1),axis=-1)                                         
    boxes1[:,3] = np.min(np.concatenate([np.expand_dims(boxes1[:,3],axis=-1),
                                         np.expand_dims(boxes2[:,3],axis=-1)],axis=-1),axis=-1)                                         
