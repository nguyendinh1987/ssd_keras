...
class AnchorBoxes_from3D(Layer):
    '''
    ...
    def __init__(self,
        ...
        super(AnchorBoxes_from3D, self).__init__(**kwargs)

    def build(self, input_shape):
        self.input_spec = [InputSpec(shape=input_shape)]
        super(AnchorBoxes_from3D, self).build(input_shape)
    ...
    def call(self, x, mask=None):
        ...
        if K.image_dim_ordering() == 'tf':
            batch_size, xx, feature_map_height, feature_map_width, feature_map_channels = x._keras_shape
        else: # Not yet relevant since TensorFlow is the only supported backend right now, but it can't harm to have this in here for the future
            batch_size, xx,feature_map_channels, feature_map_height, feature_map_width = x._keras_shape
    ...
    def compute_output_shape(self, input_shape):
        if K.image_dim_ordering() == 'tf':
            batch_size, xx,feature_map_height, feature_map_width, feature_map_channels = input_shape
        else: # Not yet relevant since TensorFlow is the only supported backend right now, but it can't harm to have this in here for the future
            batch_size, feature_map_channels, feature_map_height, feature_map_width = input_shape
        return (batch_size,xx, feature_map_height, feature_map_width, self.n_boxes, 8)
    ...
    def get_config(self):
        ...
        base_config = super(AnchorBoxes_from3D, self).get_config()
